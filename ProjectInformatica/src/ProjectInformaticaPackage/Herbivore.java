...
if (this.isMale()) {
	...
	if (Information.getLinkedListOfHerbivores().size()>1) {
		...
		for (Iterator<Herbivore> current = Information.getLinkedListOfHerbivores().iterator(); current.hasNext(); ) {
			...
			if ((currentHerbivore.isChild())&&(!currentHerbivore.isMale())) {numberFemaleChildren++;}   //смотрим, есть ли "несовершеннолетние" траво€дные самки
...
/*private void feelKillInstinct() {     // “раво€дные не убивают сородичей
...
private int getDecision() {
	...
	Arrays.sort(wish);        //сортируем желани€ траво€дного по степени важности
	...
	if ((decision==1)||(decision==2)||(decision==3)) {
		...
		if (!this.doActionWithNumber(wish[0].getNumberOfAnimalNeed())) {
			if (!this.doActionWithNumber(wish[1].getNumberOfAnimalNeed())) {
				...
				this.previousAction=(char) wish[1].getNumberOfAnimalNeed();        //потому что всегда если животное дошло до последнего действи€, то оно- сон. (всегда возвращает true)
		}
...
/*ќсуществл€етс€ проверка каждый раз, когда принимаетс€ решение.
 * ≈сли р€дом есть хищники, траво€дный убегает от них.
 * ћетод возвращает true если нужно убегать и false если не нужно.
 */
